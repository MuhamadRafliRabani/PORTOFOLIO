...
const CardLayout = ({ children, ...props }: CardLayoutProps) => {
  ...
  return (
    <div
      {...props}
      className={`GT-america relative inline-block h-fit min-h-20 w-fit  min-w-60 space-y-2 rounded-lg border border-primary bg-[#161616] p-4 uppercase tracking-wider opacity-0 shadow-xl ${id === "about-card" ? "max-w-60" : "max-w-72"}`}
    >
      {children}
